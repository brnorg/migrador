...
Quando um arquivo renderizado dentro de `.github/workflows/` substitui um arquivo diferente que ja existe no repositorio, a versao anterior e preservada com o sufixo `_m` antes da extensao. Por exemplo, antes de atualizar `.github/workflows/config.yml`, a CLI salva seu conteudo atual como `.github/workflows/config_m.yml`. Arquivos fora de `.github/workflows/`, arquivos novos ou com conteudo identico nao geram backup. Se o arquivo `_m` ja existir no repositorio ou tambem for gerado pelo template, nenhum novo backup e criado e a execucao continua normalmente.
